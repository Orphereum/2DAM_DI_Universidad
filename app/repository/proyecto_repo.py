...
class ProyectoRepository:    
    ...
    def asignar_subvencion(self, id_proyecto, id_subvencion, importe):
        ...
        cursor.execute("""
        SELECT 1 FROM proyecto_subvencion
        WHERE id_proyecto = ? AND id_subvencion = ?
        """, (id_proyecto, id_subvencion))

        existe = cursor.fetchone()

        if existe:
            # si ya existe → actualizar importe
            cursor.execute("""
                UPDATE proyecto_subvencion
                SET importe_asignado = ?
                WHERE id_proyecto = ? AND id_subvencion = ?
            """, (importe, id_proyecto, id_subvencion))
        else:
            # si no existe → insertar
            cursor.execute("""
                INSERT INTO proyecto_subvencion (id_proyecto, id_subvencion, importe_asignado, fecha_asignacion)
                VALUES (?, ?, ?, DATE('now'))
            """, (id_proyecto, id_subvencion, importe))
