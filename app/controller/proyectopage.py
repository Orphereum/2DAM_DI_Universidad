...
class ProyectoPage(QWidget):
    def __init__(self, proyecto_service, grupoInv_service, parent=None):
        ...
        # Boton asignar subvención
        self.ui.btn_asignar_subvencion.clicked.connect(self.asignar_subvencion)
    ...
    def generar_tabla(self, tabla, datos):
        # CASO ESPECIAL: SUBVENCIONES
        if tabla == self.tabla_subvenciones:
            tabla.setRowCount(len(datos))
            for fila, registro in enumerate(datos):
                nombre = registro[1]
                importe = f"{registro[2]:,.0f} €".replace(",", ".")
                
                tabla.setItem(fila, 0, QTableWidgetItem(nombre))
                tabla.setItem(fila, 1, QTableWidgetItem(importe))
            return
    ...
    def asignar_subvencion(self):
        # comprobar proyecto seleccionado
        if self.id_proyecto is None:
            QMessageBox.warning(self, "Error", "Selecciona un proyecto")
            return

        # obtener subvenciones disponibles
        subvenciones = self.proyecto_service.obtener_subvenciones_disponibles()

        if not subvenciones:
            QMessageBox.information(self, "Info", "No hay subvenciones disponibles")
            return

        # ⚡ versión simple (para que funcione ya)
        id_subvencion = subvenciones[0][0]

        # importe fijo de prueba (luego puedes mejorarlo)
        importe = 10000

        self.proyecto_service.asignar_subvencion(
            self.id_proyecto,
            id_subvencion,
            importe
        )
        ...
        QMessageBox.information(self, "OK", "Subvención asignada correctamente")
